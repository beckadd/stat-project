...
Based on our sample, we see that on average, there is a 0.42% chance of a systemic crisis occurring in any given year for a non-independent country, while there is a 9.85% chance of a systemic crisis occurring in any given year for a independent country. The difference in these proportions is 0.094320737. 

We'd like to conduct a hypothesis test to see if there is a significant difference in the proportion of years with systemic crises between independent and non-independent countries across all African countries. Our null hypothesis is that the proportion of years with systemic crises between independent and non-independent African countries is the same; the observed difference is due to chance. Our alternative hypothesis is that the proportion of years with systemic crises between independent and non-independent African countries is different. 

Since we're testing for independence, we'll use permutation. We'll modify our dataset slightly by factoring success into a categorical variable so that it'll work nicely with infer.
...
Since our p-value of 0 is less than our significance level of 0.05, we reject the null hypothesis. The data provides convincing evidence that there is a difference in the proportion of years with systemic crises between non-independent and independent African countries.
...
We set out to see if this is claim is true; that is, if there's a noticeable difference in economic stability and prosperity between North African and sub-Saharan African countries. We can evaluate these claims by analyzing our data on each country's GDP and the proportion of years with systemic crises for each country.
...
rmg <- regional_mean_gdps %>%
  pull(avg_gdp)
```
The average GDP for North African countries is `r rmg[1]`; the average GDP for sub-Saharan countries is `r rmg[2]`. Therefore, the difference in median GDP between North African and sub-Saharan countries is `r abs(rmg[1] - rmg[2])`.
...
Since our p-value of 0.528 is greater than our significance level of 0.05, we fail to reject the null hypothesis. The data does not provide convincing evidence that there is a significant difference between the median GDP of North African countries comapred to sub-Saharan countries.
...
First and foremost, we must find the difference in GDP growth between every year in the dataset, for each country.
...
Now we can perform our backwards step function, optimizing for a higher value for AIC.

```{r step-func-AIC, output = "hide"}
